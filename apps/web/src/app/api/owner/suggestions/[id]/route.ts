...
export async function PATCH(
  ...
  try {
    ...
    // Fetch the suggestion to verify ownership of the place
    ...
    // Verify the user owns the place
    const isOwner = await isOwnerOfPlace(auth.user.id, suggestion.place_id);
    if (!isOwner) {
      return NextResponse.json(
    ...
    if (updateError) {
      console.info('[owner/suggestions] Update error:', updateError.message);
      return NextResponse.json(
    ...
    if (status === 'approved') {
      ...
      if (Object.keys(placeUpdate).length > 1) {
        ...
        if (placeError) {
          console.info('[owner/suggestions] Place update error:', placeError.message);
          // Don't fail the whole request — suggestion status is already updated
        }
      }
    }
    ...
    console.info('[owner/suggestions] API error:', error);
    return NextResponse.json(
