...
export default function DashboardSuggestionDetailPage() {
  ...
  const handleReview = async (status: 'approved' | 'rejected') => {
    ...
    try {
      const res = await fetch(`/api/owner/suggestions/${suggestion.id}`, {
        method: 'PATCH',
      ...
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">
            {isClosureReport ? 'Closure Report' : 'Suggested Update'}
          </h1>
          ...
          <Card>
            <CardContent className="pt-6">
              <p className="text-sm text-gray-600 mb-4">
                {isClosureReport
                  ? 'If this closure report is valid, approving it will flag this for admin review.'
                  : 'Approving will apply the proposed changes to your place.'}
              </p>
